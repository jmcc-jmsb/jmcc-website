...
| Minors under 14 | `src/pages/privacy.astro` | ⚠ Decision needed, **deliberately not written yet**. Law 25 requires parental consent for a minor under 14. The audience is university students, but the contact form is open to anyone. Held pending the exec/team conversation — add a clause or consciously decide to omit it, do not leave it undecided |
| Photo takedown process | `src/pages/privacy.astro` → "Photos and video" | ⚠ The page now commits to removing a photo on request, without asking for justification. **Nothing defines who handles that request or how fast.** Pair it with the retention decision below and name an owner |
| Mailing list unsubscribe | `src/pages/privacy.astro` → "Our mailing list" | ⚠ The page commits to CASL terms (opt-in only, sender ID, working unsubscribe in every message). Verify the HubSpot list is configured that way before the first send — the commitment is live even though `mailingListUrl` is not |
