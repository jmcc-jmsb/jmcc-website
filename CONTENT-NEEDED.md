...
| **Discipline descriptions, EN + FR (40 entries)** | `src/data/competitions.json` → `disciplines.items[].description` | ⚠ Drafted, needs VP Academics sign-off — all 40 written, no `TODO` left. The 15 Jeux du Commerce entries are adapted from [jeuxducommerce.ca/fr/volets](https://jeuxducommerce.ca/fr/volets/). ✅ The 7 **Management Symposium** entries were rewritten 2026-08-18 from the official SMNG Academic Guide and are now the best-sourced set — see `_smng_guide` in the data file. ⚠ **Financial Open and Happening Marketing are still written from the discipline name alone** and are the ones left to check; both publish their cases as PDFs with no on-page blurbs, so an academic guide from each is what would close them. FR is machine-drafted |
