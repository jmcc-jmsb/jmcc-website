...
| ~~Nationals/internationals competition list~~ | `src/data/competitions.json` → `internationals.items` | ✅ Resolved 2026-08-18 — names, hosts, cities, URLs, logos and now **all 6 blurbs** are in (7 competitions, grouped Fall 2026 / Winter 2027). Blurbs were written from each competition's own site; see the `_blurbs` note in the file. ⚠ They carry live figures (edition number, university and country counts, case lengths) that go stale — **re-check each season**. ⚠ **BBICC's site was still down on 2026-09-14** (503; the alternate `bbicc.org` is suspended at its registrar), so its `url` is now `null` and the card shows no Visit link. Its blurb is sourced from the Faculty of Organizational Sciences' 2026 write-up. Put the URL back once the 2027 edition's site loads — see `_url_note` |
...
| ~~Mailing list sign-up~~ | `src/data/site.json` → `newsletter` | ✅ Resolved 2026-09-16 — our own `/newsletter` page posts to the HubSpot form, with a required consent box tied to the Monthly Newsletter subscription. See MAINTENANCE.md → Newsletter |
...
| Mailing list unsubscribe | `src/pages/privacy.astro` → "Our mailing list" | ⚠ Partly done 2026-09-16 — HubSpot access is in hand, and new sign-ups now record explicit consent to the Monthly Newsletter subscription. Still to check before the first send: every marketing email carries a working unsubscribe link, and the existing contacts on the list all opted in (the subscription is described as "All contacts") |
