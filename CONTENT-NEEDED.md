...
| **Discipline descriptions, EN + FR (33 entries)** | `src/data/competitions.json` → `disciplines.items[].description` | Pending — **the largest remaining content ask**, all 33 are `TODO`. Names, categories and mappings are in. Until written, `/disciplines` shows name + "Runs at" only, and the Home hover reveal has nothing to reveal. Worth splitting across the VP Academics team |
| Happening Marketing discipline list | `src/data/competitions.json` → each discipline's `competitions[]` | Pending — the only competition with no disciplines. Its `/disciplines` section renders with a "being confirmed" line so its deep link still lands somewhere real |
...
| ~~Discipline categories + assignments~~ | `src/data/competitions.json` → `disciplines.categories` | ✅ Resolved — two categories, Academic (29) and Involvement (4). Jump nav is live |
| ~~Which competition runs which discipline~~ | `src/data/competitions.json` → `competitions[]` | ✅ Resolved — all 33 mapped; drives the "Runs at" line and the `?comp=` filter |
